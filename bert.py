...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        # Calculates the attention scores by taking the dot product of query and key
        ...
        # Apply dropout to probabilities
        attention_probs = self.dropout(attention_probs)

        # Calculate the weighted sum of the value vectors
        context = torch.matmul(attention_probs, value)

        # Transpose the context tensor to match the original input shape
        context = context.transpose(1, 2).contiguous()

        # Reshape the context tensor to [b_size, seq_len, all_head_size]
